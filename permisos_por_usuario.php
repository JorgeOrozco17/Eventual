...
$user_id = isset($_GET['id_usuario']) ? intval($_GET['id_usuario']) : ($usuarios[0]['id'] ?? 0);

// Permisos actuales del usuario
$permisos = [];
if ($user_id) {
    $permisos = $userController->getPermisosPorUsuario($user_id);
}

// Agrupar páginas por categoría
$paginas_por_categoria = [];
foreach ($paginas as $pagina) {
    $categoria = $pagina['categoria'] ?? 'Sin categoría'; // Usar null coalescing operator para seguridad
    $paginas_por_categoria[$categoria][] = $pagina;
}

// Nota importante: Aquí en tu código original, el user_id del POST venía de $_GET.
// Lo he corregido para que siempre tome el user_id del POST, que es lo esperado al enviar el formulario.
                        ...
                        <div class="card-header border-0 pt-6">
                            <div class="card-title">
                                <h2 class="d-flex align-items-center">
                                    <i class="fas fa-users-cog fs-3 me-2"></i>Permisos por Usuario
                                </h2>
                            </div>
                        </div>
                        <div class="card-body pt-0">
                            ...
                            <form method="get" class="mb-8">
                                <div class="fv-row mb-5">
                                    <label for="id_usuario" class="form-label fs-6 fw-bold mb-3">Selecciona un Usuario:</label>
                                    <select name="id_usuario" id="id_usuario" class="form-select form-select-solid" data-control="select2" data-hide-search="true" onchange="this.form.submit()">
